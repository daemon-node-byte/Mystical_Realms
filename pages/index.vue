...
const servicesElem = useTemplateRef<HTMLDivElement>("services");
const scroll = useScroll(servicesElem);
const startJourney = () => {
  servicesElem.value?.scrollIntoView({ behavior: "smooth" });
};
onMounted(() => {
  $anime({
    targets: ".cta-text p",
...
<template>
  <div>
    ...
    <div ref="services" class="h-screen">
      <h3 class="text-center mt-14 text-3xl font-bold">Explore and Discover</h3>
    </div>
  </div>
</template>
